<template>
  ...
  >
    ...
    <div
      v-else
      class="pt-4 pb-10 sm:py-4 text-center border-t bg-gray-100 border-slate-200/60 dark:border-darkmode-400"
    >
      <p class="text-base">
        You have blocked messages from this account. You won't be able to text
        each other
      </p>
      <button v-if="user._id === conversationStore.detailConversation.whoBlock"
        class="mt-1 w-24 h-8 bg-slate-300 hover:bg-slate-400 rounded-lg text-base text-red-500"
        @click="unBlockChatactive"
      >
        Unblock
      </button>
    </div>
  ...
  setup(props) {
    ...
    watch(
      ...
      async (detailConversation: Conversation) => {
        ...
        const container: HTMLElement | any = await document.getElementById(
          "message-container",
        );
    ...
    async function unBlockChatactive() {
      const data = {
